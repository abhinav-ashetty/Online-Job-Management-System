...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['trendings'] = self.model.objects.filter(created_at__month=timezone.now().month)[:10]
        return context
...
class JobDetailsView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        self.object = self.get_object()  # get Job instance
        form = ApplyJobForm(request.POST)

        if Applicant.objects.filter(user=request.user, job=self.object).exists():
            messages.info(request, "You have already applied for this job.")
            return redirect('jobs:jobs-detail', id=self.object.id)

        if form.is_valid():
            applicant = form.save(commit=False)
            applicant.user = request.user
            applicant.job = self.object
            applicant.save()
