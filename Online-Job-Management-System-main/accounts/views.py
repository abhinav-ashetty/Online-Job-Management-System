...
def generate_resume(request, user_id):
    user = get_object_or_404(User, id=user_id)
    return render(request, 'jobs/employee/tr_res2.html', {'user': user})
...
def send_email(request):
    if request.method == 'POST':
        ...
        message = f"A new job has been posted:\nCompany name: {job_company}\nJob Type: {job_type}\nDescription: {job_description}\nLocation: {job_location}\nLast Date: {last_date} \nApply using this URL: {apply_url}"
        from_email = settings.EMAIL_HOST_USER
        recipient_list = ["originalemail@gmail.com", "original email@gmail.com", "original email@gmail.com"] # here you can add the email addresses of the clients you want to send the email to
